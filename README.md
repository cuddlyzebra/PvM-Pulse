...
1. Grab the latest release - either the
   **[direct link above](https://github.com/cuddlyzebra/PvM-Pulse/releases/tag/v1.0.0-beta.1)**
   or the [Releases page](../../releases) (useful once newer versions
   exist) - either:
   - **`PvM Pulse Setup <version>.exe`** - a normal installer (Start Menu
     shortcut, uninstaller, pick your install folder), or
   - **`PvM Pulse <version>.exe`** - **portable**, no install step, just
...
Bundles Node, Electron, and everything the app needs into standalone
binaries under `/release`. On Windows this produces:

- `PvM Pulse Setup <version>.exe` - installer
- `PvM Pulse <version>.exe` - portable, single-file

The first run takes longer than `npm install`, since `electron-builder`
downloads its own packaging tools (NSIS, etc.) - needs internet access,
